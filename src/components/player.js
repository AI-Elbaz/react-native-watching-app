...
const Player = ({ episode, handleIncomingVideo, sheetRef }) => {
  ...
  const { options, setAvailableQualities, availableQualities } = useContext(PlayerOptionsContext);

  const _getData = async (episode) => {
    const response = await fetch(
      "https://www.luxubu.review/api/source/" + episode.identifier,
      { method: "POST" }
    );
  ...
  useEffect(() =>
    ...
    , [videoLoading, currentTime]);

  useEffect(() => {
    if (fullscreen) Orientation.lockToLandscape();
  ...
  const _handleVideoLoaded = (data) => {
    ...
    if (currentTime) videoRef.current.seek(currentTime);
  ...
  const _handleVideoEnd = () => {
    if (options.autoPlay) _handleIncomingVideo('next');
