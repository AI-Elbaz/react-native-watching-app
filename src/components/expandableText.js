...
const ExpandableText = ({ children, numberOfLines, animationDuration = 200, style }) => {
  ...
  const height = heightAnimation.interpolate({
    ...
    outputRange: [animation[0] * textStyle.lineHeight, animation[1] * textStyle.lineHeight]
  });
  ...
  const _toggleLines = () => {
    ...
    if (lines === realLines) setCurrentLines(lines);
  ...
  return (
    <>
      <Text
        ...
        style={[textStyle, { position: 'absolute', left: Dimensions.get('window').width }]}>
        {children}
      </Text>

      <Pressable onPress={realLines > numberOfLines ? _toggleLines : null}>
        <Animated.Text style={[textStyle, { height }]} numberOfLines={currentLines}>{children}</Animated.Text>
        {realLines > numberOfLines && <Text style={styles.toggleBtn}>{currentLines === numberOfLines ? 'show more' : 'show less'}</Text>}
      </Pressable>
    </>
  );
}
